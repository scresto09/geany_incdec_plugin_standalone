...
static gboolean on_change_number (gint step)
{
	...
	for (i=pos, prev_i = -1; i >= line_start && i != prev_i; prev_i = i, i = sci_get_position_before(sci, i)) {
		...
		if ((hexa_state == HEXA_NONE || hexa_state == HEXA_REQUIRE_x) && (decimal_sate == DECIMAL_NONE || decimal_sate == DECIMAL_FOUND)) break;
	}
	...
	if (hexa_state == HEXA_FOUND || decimal_sate == DECIMAL_FOUND)
	{
		...
		if ((positive == FALSE && guessed_number >= 0) || (positive == TRUE && guessed_number < 0)) {
			...
			if (format_length > 12) format_length = 0;
		}
...
{
	if (response != GTK_RESPONSE_OK || plugin_data._entry == NULL) return;

	gint increment = gtk_spin_button_get_value(GTK_SPIN_BUTTON(plugin_data._entry));

	if (increment) on_change_number(increment);
}
...
static void on_configure_response(GtkDialog *dialog, gint response, gpointer user_data)
{
	...
	if (save_config() != TRUE) dialogs_show_msgbox(GTK_MESSAGE_ERROR, _("Plugin configuration directory could not be saved."));
